# About Me:
I design and ship backend systems end to end: APIs, event-driven microservices, and the data layer underneath them. Backend is where I go deep, but I work full-stack when the product needs it, and I like owning things all the way into production.
 
Most of my work lives in private repos, so this profile is a sample rather than the whole picture. Projects live on my [portfolio](https://portafoliotetsu.web.app/).
 
[![LinkedIn](https://img.shields.io/badge/LinkedIn-%230077B5.svg?logo=linkedin&logoColor=white)](https://www.linkedin.com/in/imertetsu/)
[![Portfolio](https://img.shields.io/badge/Portfolio-%23000000.svg?logo=firefoxbrowser&logoColor=white)](https://portafoliotetsu.web.app/)
 
---
 
## Focus
- Backend architecture: microservices, REST APIs, event-driven systems
- Data: relational modelling, query performance, multi-tenancy
- Cloud and infrastructure: containers, CI/CD, keeping services healthy in production
- Industrial IoT: connecting physical equipment to the cloud
- AI integration: building LLM capabilities into real products
---

## Tech stack
 
**Languages**
![Java](https://img.shields.io/badge/Java-%23ED8B00?logo=openjdk&logoColor=white)
![TypeScript](https://img.shields.io/badge/TypeScript-%233178C6?logo=typescript&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-%23F7DF1E?logo=javascript&logoColor=%233A3A42)
![Python](https://img.shields.io/badge/Python-%233776AB?logo=python&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-%23CC2927?logo=postgresql&logoColor=white)
![Bash](https://img.shields.io/badge/Bash-%234EAA25?logo=gnubash&logoColor=white)
 
**Backend**
![Spring Boot](https://img.shields.io/badge/Spring%20Boot-%236DB33F?logo=springboot&logoColor=white)
![Spring Security](https://img.shields.io/badge/Spring%20Security-%236DB33F?logo=springsecurity&logoColor=white)
![Hibernate](https://img.shields.io/badge/Hibernate-%2359666C?logo=hibernate&logoColor=white)
![Django](https://img.shields.io/badge/Django-%23092E20?logo=django&logoColor=white)
![Node.js](https://img.shields.io/badge/Node.js-%23339933?logo=nodedotjs&logoColor=white)
 
**Data**
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-%234169E1?logo=postgresql&logoColor=white)
![Redis](https://img.shields.io/badge/Redis-%23FF4438?logo=redis&logoColor=white)
![MongoDB](https://img.shields.io/badge/MongoDB-%2347A248?logo=mongodb&logoColor=white)
![MySQL](https://img.shields.io/badge/MySQL-%234479A1?logo=mysql&logoColor=white)
 
**Messaging and IoT**
![Kafka](https://img.shields.io/badge/Kafka-%23231F20?logo=apachekafka&logoColor=white)
![RabbitMQ](https://img.shields.io/badge/RabbitMQ-%23FF6600?logo=rabbitmq&logoColor=white)
![MQTT](https://img.shields.io/badge/MQTT-%23660066?logo=mqtt&logoColor=white)
![ThingsBoard](https://img.shields.io/badge/ThingsBoard-%23305680?logo=thingsboard&logoColor=white)
 
**Cloud and DevOps**
![AWS](https://img.shields.io/badge/AWS-%23FF9900?logo=amazonwebservices&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-%232496ED?logo=docker&logoColor=white)
![Kubernetes](https://img.shields.io/badge/Kubernetes-%23326CE5?logo=kubernetes&logoColor=white)
![GitHub Actions](https://img.shields.io/badge/GitHub%20Actions-%232088FF?logo=githubactions&logoColor=white)
![Nginx](https://img.shields.io/badge/Nginx-%23009639?logo=nginx&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-%23FCC624?logo=linux&logoColor=black)
 
**Frontend**
![React](https://img.shields.io/badge/React-%2361DAFB?logo=react&logoColor=black)
![Angular](https://img.shields.io/badge/Angular-%23DD0031?logo=angular&logoColor=white)
![RxJS](https://img.shields.io/badge/RxJS-%23B7178C?logo=reactivex&logoColor=white)
![TailwindCSS](https://img.shields.io/badge/TailwindCSS-%2306B6D4?logo=tailwindcss&logoColor=white)
 
---
 
# GitHub Stats:
![](https://github-readme-stats.vercel.app/api?username=imertetsu&theme=merko&hide_border=false&include_all_commits=true&count_private=true)<br/>
![](https://github-readme-streak-stats.herokuapp.com/?user=imertetsu&theme=merko&hide_border=false)<br/>
![](https://github-readme-stats.vercel.app/api/top-langs/?username=imertetsu&theme=merko&hide_border=false&include_all_commits=true&count_private=true&layout=compact)
<!-- Proudly created with GPRM ( https://gprm.itsvg.in ) -->
